...
// DestinationTypeCoordinator is responsible for setup task managers and publish coming process events using suitable task publishers.
...
func (d DestinationTypeCoordinator) Start(processedEvents <-chan event.ProcessedEvent, done <-chan bool, wg *sync.WaitGroup) error {
	...
	go func() {
		defer wg.Done()
		log.Println("Starting destination type task coordinator...")
		for {
			select {
			case pe := <-processedEvents:
				...
				if !ok {
					fmt.Println("Error on getting task manager for ", pe.DestinationType())
